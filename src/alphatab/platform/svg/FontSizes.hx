...
class FontSizes 
{
    ...
    public static function measureString(s:String, f:SupportedFonts, size:Float)
    {
        ...
        for (i in 0 ... s.length) 
        {
            var code:Int = Std.int(Math.min(data.length - 1, s.charCodeAt(i))) - ControlChars;
            if (code >= 0) {
