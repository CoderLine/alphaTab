...
import {
    ...
    HydraPhdr,
    HydraShdr
} from '@src/audio/synth/soundfont/Hydra';
import { Channel } from '@src/audio/synth/synthesis/Channel';
import { Channels } from '@src/audio/synth/synthesis/Channels';
import { LoopMode } from '@src/audio/synth/synthesis/LoopMode';
import { OutputMode } from '@src/audio/synth/synthesis/OutputMode';
import { Preset } from '@src/audio/synth/synthesis/Preset';
import { Region } from '@src/audio/synth/synthesis/Region';
import { SynthEvent } from '@src/audio/synth/synthesis/SynthEvent';
...
export class TinySoundFont {
    ...
    private fillWorkingBuffer(silent: boolean): Float32Array {
        ...
        Platform.clearIntArray(this._midiEventCounts);
        return buffer;
    }
    ...
    private processMidiMessage(e: MidiEvent): void {
        ...
        switch (command) {
            ...
            case MidiEventType.NoteOn:
                this.channelNoteOn(channel, data1, data2 / 127);
                break;
    ...
    private channelApplyPitch(channel: number, c: Channel): void {
        const pitchShift: number =
            c.pitchWheel === 8192 ? c.tuning : (c.pitchWheel / 16383.0) * c.pitchRange * 2 - c.pitchRange + c.tuning;
        for (const v of this._voices) {
            if (v.playingChannel === channel && v.playingPreset !== -1) {
