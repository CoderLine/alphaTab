...
class Gp5Reader extends GpReaderBase
{
	...
	private function ReadBeat(start:Int, measure:GsMeasure, track:GsTrack, voiceIndex:Int) : Int
	{
        ...
        var effect:GsNoteEffect = new GsNoteEffect();
        if ((flags & 0x02) != 0) {
		...
		for (j in 0 ... 7)
		{
			...
			if ((stringFlags & (1 << i)) != 0 && (6 - i) < track.StringCount()) {
                ...
                var note:GsNote = this.ReadNote(guitarString, track, effect.Clone(Factory));
                voice.AddNote(note);
            }
	...
	private function ReadBeatEffects(beat:GsBeat, effect:GsNoteEffect)  : Void
	{
        ...
        beat.HasRasgueado = (flags2 & 0x1) != 0;
        if ((flags2 & 0x02) != 0) {
            beat.PickStroke = ReadByte();
