...
export class ScoreLoader {
     ...
     * @returns Promise of Score object from datasource
     * @target web
     */
            ...
            xhr.onreadystatechange = () => {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    ...
                    if (xhr.status === 200 || (xhr.status === 0 && response)) {
                        ...
                        reject(new FileLoadError('You are offline!!\n Please Check Your Network.', xhr));
                    } else if (xhr.status === 404) {
                        reject(new FileLoadError('Requested URL not found.', xhr));
                    } else if (xhr.status === 500) {
                        reject(new FileLoadError('Internel Server Error.', xhr));
                    } else if (xhr.statusText === 'parsererror') {
                        reject(new FileLoadError('Error.\nParsing JSON Request failed.', xhr));
                    } else if (xhr.statusText === 'timeout') {
                        reject(new FileLoadError('Request Time out.', xhr));
                    } else {
                        reject(new FileLoadError('Unknow Error: ' + xhr.responseText, xhr));
                    }
                }
            };
            xhr.send();
