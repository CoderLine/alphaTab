...
export default class CSharpAstTransformer {
    ...
    public constructor(typeScript: ts.SourceFile, context: CSharpEmitterContext) {
        ...
        let fileName = path.relative(path.resolve(this._context.compilerOptions.baseUrl!), path.resolve(this._typeScriptFile.fileName));
        ...
        this._typeScriptFile.statements.forEach(s => {
            if (ts.isClassDeclaration(s) || ts.isInterfaceDeclaration(s) || ts.isEnumDeclaration(s)) {
                ...
                if (ts.isFunctionTypeNode(s.type)) {
                    ...
                    this._context.addTsNodeDiagnostics(s, "Only FunctionType type aliases are allowed", ts.DiagnosticCategory.Error);
                }
            }
        ...
        if (path.basename(this._typeScriptFile.fileName).toLowerCase() === 'alphatab.ts') {
            ...
            if (!defaultExport || !ts.isClassDeclaration(defaultExport)) {
                globalStatements.forEach(s => {
                    this._context.addTsNodeDiagnostics(s, "Global statements in modules are only allowed if there is a default class export", ts.DiagnosticCategory.Error);
                });
            ...
            let folders = path.dirname(path.relative(path.resolve(this._context.compilerOptions.baseUrl!), path.resolve(this._typeScriptFile.fileName))).split(path.sep);
            // TODO: make default root folders configurable from outside
            if (folders.length > 0 && (folders[0] === 'src' || folders[0] === 'test')) {
                folders.shift();
            }
            this._csharpFile.namespace.namespace = 'AlphaTab' + folders.map(f => '.' + f.substr(0, 1).toUpperCase() + f.substr(1)).join();

            if (defaultExport) {
                this.visit(defaultExport, additionalNestedExportDeclarations, additionalNestedNonExportsDeclarations, globalStatements);
            }
    ...
    private visit(node: ts.Node, additionalNestedExportDeclarations?: ts.Declaration[], additionalNestedNonExportsDeclarations?: ts.Declaration[], globalStatements?: ts.Statement[]): ts.Node {
        if (ts.isClassDeclaration(node)) {
            this.visitClassDeclaration(node, additionalNestedExportDeclarations, additionalNestedNonExportsDeclarations, globalStatements);
        }
    ...
    private visitInterfaceDeclaration(node: ts.InterfaceDeclaration) {
        ...
        if (extendsClauses && extendsClauses.length > 0) {
            csInterface.interfaces = extendsClauses.map((n: ts.Node) => <cs.TypeNode>{
    ...
    private visitClassDeclaration(node: ts.ClassDeclaration, additionalNestedExportDeclarations?: ts.Declaration[], additionalNestedNonExportsDeclarations?: ts.Declaration[], globalStatements?: ts.Statement[]) {
        ...
        if (extendsClause) {
            csClass.baseClass = {
                nodeType: cs.SyntaxKind.UnresolvedTypeNode,
                tsNode: extendsClause,
                parent: csClass
            };
        }

        if (implementsClauses && implementsClauses.length > 0) {
            csClass.interfaces = implementsClauses.map((n: ts.Node) => <cs.TypeNode>{
                nodeType: cs.SyntaxKind.UnresolvedTypeNode,
                tsNode: n,
                parent: csClass
            });
    ...
    private visitClassElement(parent: cs.ClassDeclaration, classElement: ts.ClassElement) {
        if (ts.isConstructorDeclaration(classElement)) {
            ...
            this._context.addTsNodeDiagnostics(classElement, "Unsupported class element: " + ts.SyntaxKind[classElement.kind], ts.DiagnosticCategory.Error);
        }
    }
    ...
    private visitInterfaceElement(parent: cs.InterfaceDeclaration, classElement: ts.TypeElement) {
        if (ts.isMethodSignature(classElement)) {
            ...
            this._context.addTsNodeDiagnostics(classElement, "Unsupported interface element: " + ts.SyntaxKind[classElement.kind], ts.DiagnosticCategory.Error);
        }
    }
   
    private visitPropertySignature(parent: cs.ClassDeclaration | cs.InterfaceDeclaration, classElement: ts.PropertySignature) {
    }

    private visitGetAccessor(parent: cs.ClassDeclaration | cs.InterfaceDeclaration, classElement: ts.GetAccessorDeclaration) {
    }

    private visitSetAccessor(parent: cs.ClassDeclaration | cs.InterfaceDeclaration, classElement: ts.SetAccessorDeclaration) {
    }

    private visitPropertyDeclaration(parent: cs.ClassDeclaration | cs.InterfaceDeclaration, classElement: ts.PropertyDeclaration) {
    }

    private visitMethodDeclaration(parent: cs.ClassDeclaration | cs.InterfaceDeclaration, classElement: ts.MethodDeclaration) {
    }

    private visitMethodSignature(parent: cs.ClassDeclaration | cs.InterfaceDeclaration, classElement: ts.MethodSignature) {
    }
