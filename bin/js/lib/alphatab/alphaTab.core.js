...
alphatab.rendering.AlternateEndingsBarRendererFactory.__name__ = ["alphatab","rendering","AlternateEndingsBarRendererFactory"];
...
alphatab.audio.generator.MidiFileGenerator = function(score) {
	this._score = score;
	this._currentTempo = this._score.tempo;
	this._midiFile = new alphatab.audio.model.MidiFile();
	this._handler = new alphatab.audio.generator.MidiFileHandler(this._midiFile);
	var _g1 = 0, _g = this._score.tracks.length + 2;
	while(_g1 < _g) {
		var i = _g1++;
		this._midiFile.createTrack();
	}
	this._midiFile.infoTrack = this._midiFile.tracks.length - 2;
	this._midiFile.metronomeTrack = this._midiFile.tracks.length - 1;
};
...
alphatab.audio.generator.MidiFileGenerator.prototype = {
	...
	,getBrushInfo: function(beat) {
		...
		var _g1 = 0, _g = beat.voice.bar.track.tuning.length;
		while(_g1 < _g) {
			var i = _g1++;
			brushInfo.push(0);
		}
	...
	,getDynamicValue: function(note) {
		...
		return allDynamics[Math.max(0,Math.min(allDynamics.length - 1,currentIndex)) | 0];
	}
	,applyStaticDuration: function(duration,maximum) {
		var value = this._currentTempo * duration / 60;
		return Math.min(value,maximum) | 0;
	}
	...
	,generateMasterBar: function(masterBar,previousMasterBar,startMove) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			this._handler.addNote(this._midiFile.metronomeTrack,start,length,37,alphatab.model.DynamicValue.F,9);
			start += length;
		}
	}
	...
	,generate: function() {
		...
		while(!(controller.index >= controller._score.masterBars.length)) {
			...
			if(controller.shouldPlay) {
				this.generateMasterBar(this._score.masterBars[controller.index],previousMasterBar,controller.repeatMove);
				var _g = 0, _g1 = this._score.tracks;
				while(_g < _g1.length) {
					var track = _g1[_g];
					++_g;
					this.generateBar(track.bars[controller.index],controller.repeatMove);
				}
			}
			previousMasterBar = this._score.masterBars[controller.index];
		}
	}
	,_currentTempo: null
	,_handler: null
...
alphatab.audio.generator.MidiFileHandler.prototype = {
	...
	,addRest: function(track,start,channel) {
	}
...
alphatab.audio.generator.MidiPlaybackController.prototype = {
	process: function() {
		...
		this._lastIndex = Math.max(this._lastIndex,this.index) | 0;
...
alphatab.audio.model.MidiEvent = function() {
};
...
alphatab.importer.GpImporterTestBase.prototype = $extend(haxe.unit.TestCase.prototype,{
	...
	,checkOtherEffects: function(score) {
		...
		this.assertTrue(score.tracks[0].bars[3].voices[0].beats[0].chordId != null,{ fileName : "GpImporterTestBase.hx", lineNumber : 311, className : "alphatab.importer.GpImporterTestBase", methodName : "checkOtherEffects"});
	...
	,checkTrills: function(score) {
		...
		this.assertEquals(alphatab.model.Duration.Sixteenth,score.tracks[0].bars[0].voices[0].beats[1].notes[0].trillSpeed,{ fileName : "GpImporterTestBase.hx", lineNumber : 289, className : "alphatab.importer.GpImporterTestBase", methodName : "checkTrills"});
...
alphatab.importer.Gp3ImporterTest.prototype = $extend(alphatab.importer.GpImporterTestBase.prototype,{
	...
	,testOtherEffects: function() {
		...
		this.assertTrue(score.tracks[0].bars[3].voices[0].beats[0].chordId != null,{ fileName : "Gp3ImporterTest.hx", lineNumber : 137, className : "alphatab.importer.Gp3ImporterTest", methodName : "testOtherEffects"});
...
alphatab.platform.svg.FontSizes.measureString = function(s,f,size) {
	...
	while(_g1 < _g) {
		var i = _g1++;
		var code = (Math.min(data.length - 1,HxOverrides.cca(s,i)) | 0) - alphatab.platform.svg.FontSizes.ControlChars;
		if(code >= 0) {
...
alphatab.rendering.AlternateEndingsBarRenderer.__name__ = ["alphatab","rendering","AlternateEndingsBarRenderer"];
...
alphatab.rendering.ScoreBarRenderer.prototype = $extend(alphatab.rendering.GroupedBarRenderer.prototype,{
	...
	,createKeySignatureGlyphs: function() {
		...
		var previousKeyPositions = previousKey > 0?alphatab.rendering.ScoreBarRenderer.SharpKsSteps:alphatab.rendering.ScoreBarRenderer.FlatKsSteps;
		var _g1 = 0;
		...
		if(currentKey > 0) {
			...
			while(_g2 < _g1) {
				var i = _g2++;
				this.addPreBeatGlyph(new alphatab.rendering.glyphs.SharpGlyph(0,this.getScoreY(alphatab.rendering.ScoreBarRenderer.SharpKsSteps[i] + offsetClef) | 0));
			}
		} else {
			var _g2 = 0, _g1 = Math.abs(currentKey) | 0;
			while(_g2 < _g1) {
				var i = _g2++;
				this.addPreBeatGlyph(new alphatab.rendering.glyphs.FlatGlyph(0,this.getScoreY(alphatab.rendering.ScoreBarRenderer.FlatKsSteps[i] + offsetClef) | 0));
			}
		}
	}
	...
	,_currentTupletHelper: null
	,_currentBeamHelper: null
	,_tupletHelpers: null
	,_beamHelperLookup: null
	,_beamHelpers: null
...
alphatab.rendering.glyphs.ScoreBeatContainerGlyph.__name__ = ["alphatab","rendering","glyphs","ScoreBeatContainerGlyph"];
...
alphatab.rendering.glyphs.ScoreBrushGlyph.prototype = $extend(alphatab.rendering.Glyph.prototype,{
	paint: function(cx,cy,canvas) {
		...
		var res = this.renderer.stave.staveGroup.layout.renderer.renderingResources;
		var startY = cy + this.y + (scoreBarRenderer.getNoteY(this._beat.maxNote()) - lineSize / 2 | 0);
...
alphatab.rendering.glyphs.effects.CrescendoGlyph.__name__ = ["alphatab","rendering","glyphs","effects","CrescendoGlyph"];
...
alphatab.rendering.glyphs.effects.PickStrokeGlyph.__name__ = ["alphatab","rendering","glyphs","effects","PickStrokeGlyph"];
...
alphatab.rendering.glyphs.effects.TrillGlyph.__name__ = ["alphatab","rendering","glyphs","effects","TrillGlyph"];
...
alphatab.util.Guid.__name__ = ["alphatab","util","Guid"];
...
alphatab.rendering.layout.PageViewLayout.PagePadding = [20,20,20,20];
...
alphatab.audio.generator.MidiFileHandler.Volume = 7;
alphatab.audio.generator.MidiFileHandler.Balance = 10;
alphatab.audio.generator.MidiFileHandler.Expression = 11;
