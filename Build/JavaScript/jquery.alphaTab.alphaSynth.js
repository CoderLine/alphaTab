    ...
    api.playerInit = function(element, context, args) {
        ...
        if(!as) {
            ...
            as.On('ready', function() {
                as.LoadSoundFontUrl(args.soundFontUrl);
            });
            as.On('soundFontLoad', function(loaded, full) {
                element.trigger('soundFontLoad', {
                    loaded: loaded,
                    full: full
                }
            });          
            ...
            element.on('loaded', function(e) {
                loadMidi(element, context, e.originalEvent.detail);
            });
            
            // initial load
            loadMidi(element, context, api.score(element, context));            
        }        
    };
