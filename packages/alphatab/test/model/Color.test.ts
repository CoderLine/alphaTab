...
describe('ColorTests', () => {
    it('fromJson-number', () => {
        ...
        expect(color!.rgba).to.equal('#F64E06');
    });

    it('fromJson-#RGB', () => {
        const color = Color.fromJson('#0f0');

        expect(color!.rgba).to.equal('#00FF00');
    });

    it('fromJson-#RGBA', () => {
        const color = Color.fromJson('#0f09');

        expect(color!.rgba).to.equal('rgba(0,255,0,0.6)');
    });
    ...
    it('fromJson-#RRGGBBAA', () => {
        ...
        expect(color!.rgba).to.equal('rgba(246,78,6,0)');
    });
