...
import { ModelUtils, type TuningParseResult } from '@coderline/alphatab/model/ModelUtils';
...
export class AlphaTexLexerOld {
    ...
    private _nextCodepoint(): number {
        if (this._position < this._codepoints.length) {
            ...
            this._codepoint = AlphaTexLexerOld._eof;
        }
    ...
    public newSy(allowFloats: boolean = false): AlphaTexSymbols {
        ...
        while (this.sy === AlphaTexSymbols.No) {
            ...
            if (this._codepoint === AlphaTexLexerOld._eof) {
                this.sy = AlphaTexSymbols.Eof;
            } else if (AlphaTexLexerOld._isWhiteSpace(this._codepoint)) {
                // skip whitespaces
                this._codepoint = this._nextCodepoint();
                this._saveValidSpot();
                ...
                if (this._codepoint === 0x2f /* / */) {
                    ...
                    // multiline comment
                    while (this._codepoint !== AlphaTexLexerOld._eof) {
                ...
                while (this._codepoint !== startChar && this._codepoint !== AlphaTexLexerOld._eof) {
                    // escape sequences
                    let codepoint = -1;
                ...
                this.syData = '<';
                this._codepoint = this._nextCodepoint();
            } else if (AlphaTexLexerOld._isDigit(this._codepoint)) {
                this._readNumberOrName(allowFloats);
            } else if (AlphaTexLexerOld._isNameLetter(this._codepoint)) {
    ...
    private _readNumberOrName(allowFloat: boolean) {
        ...
        if (this._codepoint === 0x2d) {
            ...
            if (!AlphaTexLexerOld._isDigit(this._codepoint)) {
                this.sy = AlphaTexSymbols.String;
            }
        }
            ...
            switch (this.sy) {
                case AlphaTexSymbols.Number:
                    ...
                    // adding a dot to the number (expecting digit after dot)
                        ...
                        AlphaTexLexerOld._isDigit(this._codepoints[this._position])
                    ) {
                        str += String.fromCodePoint(this._codepoint);
                        this._codepoint = this._nextCodepoint();
                        keepReading = true;
                ...
                case AlphaTexSymbols.String:
                    if (AlphaTexLexerOld._isNameLetter(this._codepoint)) {
                        str += String.fromCodePoint(this._codepoint);
                        this._codepoint = this._nextCodepoint();
                        keepReading = true;
                    } else {
    ...
    private _readName(): string {
        ...
        do {
            ...
            AlphaTexLexerOld._isNameLetter(this._codepoint) ||
            AlphaTexLexerOld._isDigit(this._codepoint) ||
    ...
    private static _isNameLetter(ch: number): boolean {
        return (
            !AlphaTexLexerOld._isTerminal(ch) && // no control characters, whitespaces, numbers or dots
...
export class AlphaTexImporterOld extends ScoreImporter {
                ...
                if (name === 'defaults') {
                    for (const [defaultName, defaultValue] of PercussionMapper.instrumentArticulationNames) {
                        ...
                        this._percussionArticulationNames.set(
                            AlphaTexImporterOld._toArticulationId(defaultName),
                            defaultValue
                        );
                    }
                    return StaffMetaResult.KnownStaffMeta;
    ...
    private _beat(voice: Voice): boolean {
        ...
        if (this._currentTuplet !== 1 && !beat.hasTuplet) {
            AlphaTexImporterOld._applyTuplet(beat, this._currentTuplet);
        }
    ...
    private _applyBeatEffect(beat: Beat): boolean {
        ...
        if (syData === 'f') {
            ...
            if (this._sy === AlphaTexSymbols.Number) {
                ...
                AlphaTexImporterOld._applyTuplet(beat, numerator);
            }
