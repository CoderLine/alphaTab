    ...
    if (typeInfo.modulePath) {
        ...
        if (isOwnType) {
            ...
            if (isGeneratedJsonDeclaration) {
                importer(`${typeInfo.typeAsString}Json`, `@coderline/alphatab/generated/${typeInfo.typeAsString}Json`);
            } else {
...
const emit = createEmitter('json_declaration', (program, input) => {
    ...
    allJsonTypes.set(`${input.name!.text}Json`, `@coderline/alphatab/generated/${input.name!.text}Json`);
