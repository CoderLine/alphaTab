...
export class AlphaTex1LanguageDefinitions {
    ...
    private static _simple(signature: (SimpleAlphaTexParameterDefinition | null)[][] | null): AlphaTexSignatureDefinition[] | null {
        if (signature == null) {
            return null;
        }
    ...
    private static _metaProps(props: [
        string,
        [
            string,
            SimpleAlphaTexParameterDefinition[][] | null
        ][] | null
    ][]) {
        return new Map(props.map(p => [
            p[0],
            p[1] === null ? null : new Map(p[1]!.map(p => [p[0], AlphaTex1LanguageDefinitions._simple(p[1])]))
        ]));
    }
    private static _props(props: [
        string,
        SimpleAlphaTexParameterDefinition[][] | null
    ][]) {
        return new Map(props.map(p => [p[0], AlphaTex1LanguageDefinitions._simple(p[1])]));
    }
    private static _signatures(signatures: [
        string,
        SimpleAlphaTexParameterDefinition[][] | null
    ][]) {
