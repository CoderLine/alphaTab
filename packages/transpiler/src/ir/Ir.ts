/**
 * Intermediate representation shared by the C# and Kotlin emitters.
 *
 * Both targets consume the same shape; per-target differences manifest
 * only at the printer and target-strategy levels. Convention:
 *
 *     import * as cs from '../ir/Ir';
 *
 * (The `cs` alias is historical — the C# target was implemented first.
 * The namespace is the canonical IR; the alias preserves diff churn cost.)
 */
...
export interface PropertyAccessorDeclaration extends Node {
    ...
    keyword: string;
    body?: Block | Expression;
    /** TS setter parameter name when it differs from the C# implicit `value`. */
...
/**
 * A LazyTypeRef *is* a TypeNode: it occupies a type slot during the
 * TS->IR transform and resolves itself on demand. The node carries
 * its own `resolve()` closure and a memoised result; there is no
 * global resolution buffer.
 *
 * On the first call to `resolve()` the node runs the standard
 * `getTypeFromTsType` cascade (closed over the
 * `CSharpEmitterContext` at construction time) and mutates itself in
 * place to mirror the resolved node's properties — so existing
 * references to the placeholder remain valid (the "no rewiring
 * needed" invariant). Subsequent calls short-circuit through
 * `resolved`.
 *
 * Every reachable `LazyTypeRef` is primed by `ResolveTypesPass`
 * before the printer runs (Invariant 1 in `ir/README.md`).
 */
export interface LazyTypeRef extends TypeNode {
    nodeType: SyntaxKind.LazyTypeRef;
    tsType: ts.Type;
    tsSymbol?: ts.Symbol;
    typeArguments?: LazyTypeRef[];
    skipAliasSymbolOnResolve?: boolean;

    /**
     * Resolution closure bound at construction. Idempotent: the first
     * call mutates `this` in place to merge the resolved type's
     * properties (`nodeType` may flip from `LazyTypeRef` to
     * `TypeReference`, `ArrayTypeNode`, etc.) and stores a
     * self-reference in `resolved`; subsequent calls return that
     * cached value.
     */
    resolve(): TypeNode;

    /**
     * Memoised result of the first `resolve()` call. After resolution
     * this points back at `this` (because the merge happens in
     * place), so a non-undefined value means "already resolved".
     */
    resolved?: TypeNode;
}
...
export interface NumericLiteral extends Node {
    ...
    value: string;
    /**
     * Kotlin-only marker set by `DoubleSuffixPass`. When true the
     * Kotlin printer appends a `.0` suffix so the literal types as a
     * `Double` in its surrounding context. The pass evaluates this once
     * per literal; the printer just reads the flag.
     */
    forceDoubleSuffix?: boolean;
}
...
export function isLazyTypeRef(node: Node): node is LazyTypeRef {
    return node.nodeType === SyntaxKind.LazyTypeRef;
}
