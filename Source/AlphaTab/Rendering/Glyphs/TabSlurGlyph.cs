...
{
    class TabSlurGlyph : TabTieGlyph
    {
        ...
        {
            // same type required
            if (ForSlide != forSlide) return false;
            if (ForEnd != forEnd) return false;
            // same start and endbeat
            if (StartNote.Beat.Id != startNote.Beat.Id) return false;
            if (EndNote.Beat.Id != endNote.Beat.Id) return false;
            // same draw direction
            if (_direction != GetBeamDirection(startNote)) return false;
